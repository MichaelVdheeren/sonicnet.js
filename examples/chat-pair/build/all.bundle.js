...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__pair_client_js__ = __webpack_require__("./chat-pair/pair-client.js");
...
var pairClient = new __WEBPACK_IMPORTED_MODULE_1__pair_client_js__["a" /* default */]();
...
function getUserName() {
  return localStorage.userName === undefined ? 'Anonymous' : localStorage.userName;
}
...
/***/ "./chat-pair/pair-client.js":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var PairClient = function () {
  function PairClient() {
    _classCallCheck(this, PairClient);
  ...
  PairClient.prototype.start = function start(token) {
    var msg = JSON.stringify({ type: 'start', token: token });
    this.socket.send(msg);
  };

  PairClient.prototype.confirm = function confirm(token) {
    var msg = JSON.stringify({ type: 'confirm', token: token });
    this.socket.send(msg);
  };

  PairClient.prototype.send = function send(message) {
    if (this.conn_id === null) {
      console.error('No connection ID.');
      return;
    }
    var msg = JSON.stringify({ type: 'message', conn_id: this.conn_id, message: message });
    this.socket.send(msg);
  };

  PairClient.prototype.on = function on(event, callback) {
    if (event === 'ready') {
  ...
  PairClient.prototype.onMessage_ = function onMessage_(e) {
    var json = void 0;
    try {
  ...
  PairClient.prototype.fire_ = function fire_(callback, arg) {
    if (callback) {
      callback(arg);
    }
  };

  PairClient.prototype.onError_ = function onError_(err) {
    console.error(err);
...
/* harmony default export */ __webpack_exports__["a"] = (PairClient);
...
module.exports = __webpack_require__("./chat-pair/index.js");
